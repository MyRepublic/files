...
function scrollSpyInit(){ 
	...
	if(location.pathname.length > 1 && $('body.home').length == 0) {
...
$(window).load(pageLoadHash);
	...
	function reLayout() {
		...
		$.each($portfolio_containers,function(i){
			...
			//non masonry
			if($portfolio_containers[i].hasClass('no-masonry') && $portfolio_containers[i].find('.col.elastic-portfolio-item:first:visible').length > 0){
				//reset height for calcs
		   	   ...
		   	   $portfolio_containers[i].find('.col.elastic-portfolio-item img').css('height',tallColHeight);
			}
